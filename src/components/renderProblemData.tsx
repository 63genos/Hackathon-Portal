...
const renderProblemData = (data: JSONData | undefined) => {
    ...
    return (
        <Box mt="40px">
            {problemStatement && (
                <div>
                    <Text fontSize={{ base: "1.5rem", md: "2rem" }} fontWeight="semibold" color="white">
                        Problem Statement
                    </Text>
                    <Text mt="12px" fontSize="1.25rem" fontWeight="normal" color="white">
                        {problemStatement}
                    </Text>
                </div>
            )}
            {otherData.map(([key, value]) => (
                <div key={key}>
                    <Text mt="48px" fontSize={{ base: "1.5rem", md: "2rem" }} fontWeight="semibold" color="white">
                        {key}
                    </Text>
                    {Array.isArray(value) ? (
